...
def saveScores( dst_path, model_name,  metrics_values ):
    ...
    file.write("File;MSE-NZ PERF;PSRN PEFT;ACC PERF;SSM PERF; HOG PERF\n")
    
    for y in range(len(metrics_values)):
        msenz_perf = metrics_values[y][1] / metrics_values[y][2] 
        ...
        line = metrics_values[y][0] + ";" + str(msenz_perf) + ";" + str( psnr_perf) + ";" + str(acc_perf) \
...
##############
        ...
        msenz_predi_current = mseNoZeros(ensemble, nitid_images[i])
        ...
        mse_predi_current = mse( nitid_images[i].flatten(), ensemble.flatten())
...
def predictByIndexesModels( models, noisy_images, nitid_images, noisy_files, nitid_files, test_indexes, accuracy_threshold, save_pred = False, \
    ...
    for i in test_indexes:
        ...
        msenz_predi = mseNoZeros(ensemble, nitid_images[i])
        ...
        mse_predi = mse( nitid_images[i].flatten(), ensemble.flatten())
