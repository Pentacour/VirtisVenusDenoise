...
def buildModel( hyperparameters):
    ...
    if not hasattr(hyperparameters, 'OPTIMIZER'):
        hyperparameters.OPTIMIZER = Adam(beta_2 = 0.9)

    HYPER = hyperparameters
    
    return build_resnet_model(HYPER.IMG_HEIGHT,HYPER.IMG_WIDTH,HYPER.NUM_CHANNELS,HYPER.NUM_RES_BLOCKS, HYPER.OPTIMIZER, HYPER.LOSS)



def build_resnet_model(height,width,num_channels,num_res_blocks, optimizer, loss_function):
